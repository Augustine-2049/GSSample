...
uniform mat4 view;
// const int MAX_INSTANCES = 16384; // 下面作为uniform传入，主函数全局修改即可
...
void main() {
    ...
    vec3 rotated_vector = quat_rotate(in_rotate, scaled_offset);
    ...
    float point_render_size = 50.0 * (in_scale.x + in_scale.y + in_scale.z) / gl_Position.w;
    gl_PointSize = max(1.0, point_render_size);
